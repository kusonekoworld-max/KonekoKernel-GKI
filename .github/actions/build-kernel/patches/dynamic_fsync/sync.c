...
static int do_fsync(unsigned int fd, int datasync)
{
	...
	/*
	 * Dynamic fsync: skip the writeback entirely while the screen is on
	 * and the feature is enabled -- don't even take the fd reference in
	 * that case, so there is no fdget()/fdput() asymmetry (unlike the
	 * upstream driver's version of this hook, which grabbed fdget()
	 * *before* this check and leaked the reference on every skip).
	 */
